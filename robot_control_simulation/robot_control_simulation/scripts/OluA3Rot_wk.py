...
def home_robot():  ### Change the home co-orddinates to my desired position###
	header = "def myProg():"
	footer = "\nend"
	# [0] = 1.58825 if move can not be generated change the home waypoint to inverse(+/-)
	home_waypoint = [1.58825, -1.71042, -2.19911, -0.802851, 1.58825, -0.03106686]
...
def rmove(rmove,angle):
	...
	if rmove == 1: #+Roll
		...
		errors = ur_con.check_errors(my_pose)
		rot2_err = errors.ee_rot_error
		print("+Roll:", (rot2_err),"r") #Rotational error		
	...
	if rmove == 3:#+Pitch
		...
		errors = ur_con.check_errors(my_pose)
		rot2_err = errors.ee_rot_error
		print("+Pitch:", (rot2_err),"r") #Rotational error			
	...
	if rmove == 5:#+Yaw
		...
		errors = ur_con.check_errors(my_pose)
		rot2_err = errors.ee_rot_error
		print("+Yaw:", (rot2_err),"r") #Rotational error	
...
###--Set Robot to home before every loopposition---###
...
if car_err > 0.05:
	while car_err > 0.05:
		thresh_car()
		thresh_car.mag = thresh_car() # WORKING
		print("Exploring" , thresh_car.mag , "m in every axis") # WORKING
		###---Issue contained here---###
...
###-------------------------WORKING ROTATIONAL CATCHING LOOP--------------------------------###

###---TRIGGERING THE ROTATIONAL LOOP---###
